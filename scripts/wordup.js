...
function addNewWordSubmission(word) {
    ...
    // TODO 21
    // replace the hardcoded 'false' with the real answer, sets true if alreadyUsed
...
function checkIfWordIsReal(word) {
    ...
    // make an AJAX call to the Pearson API - lasde
    $.ajax({
        // TODO 13 what should the url be?
        url: "http://api.pearson.com/v2/dictionaries/lasde/entries?headword=" + word,
        success: function(response) {
            console.log("We received a response from the Pearson Dictionary!");
            ...
            model.wordSubmissions.forEach(function(single){
                if (single.word == word){
                  single.isRealWord = theAnswer;
                }
              })
              ;
...
function render() {
    ...
    console.log(letterChips);
    $("#allowed-letters").append(letterChips);

    // TODO 11
    // Render the word submissions

    var  wordz = model.wordSubmissions.map(wordSubmissionChip);
    //console.log(wordSubChips)
    $("#word-submissions").append(wordz);
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        ...
        // TODO 17
        var gameScore = wordScore(wordSubmission.word);
        ...
        if( wordSubmission.isRealWord == true ) {
            scoreChip.text(gameScore);
            scoreChip.attr("style", "font-weight: bold; padding: 2px; margin: 5px; font-size: 13px; background-color: green;");
        }
        else {
            scoreChip.text("X");
            scoreChip.attr("style", "padding: 2px; margin: 5px; font-size: 13px; font-weight: bold; background-color: red;");
        }
...
function currentScore() {
    ...
    //var totalz = 0;
    //for(var w=0; w < model.wordSubmissions.length; w++) {
    //    totalz += wordScores[w];
    //}
    
    //return totalz;
    //using reduce
    var totalz = wordScores.reduce(add, 0);
    console.log(totalz)
    return totalz
